...
/**
 * ProjectName: EnhancedSwipeRefreshLayout
 * Description:
 * <p>
 * 自定义SwipeRefreshLayout<br>
 * 支持下拉刷新和上拉加载更多<br>
...
public class SwipeRefreshLayout extends ViewGroup {
    ...
    // SuperSwipeRefreshLayout包括三个view：headerView, mTarget, footerView
    // mTarget：一般指是随着下拉刷新，上拉加载的效果，而变化的view，通常容器view
    // 比如RecyclerView, ListView, ScrollView, GridView, RelativeLayout etc.
    ...
    private int mHeaderViewWidth;           // headerView的宽度 = screen width
    private int mHeaderViewHeight;          // headerView的高度 = 50dp

    private int mFooterViewWidth;           // footerView的宽度 = screen width
    private int mFooterViewHeight;          // footerView的高度 = 50dp
    ...
    private boolean isCanPull = true;               // 处理当前View中的各种状态时，判断是否允许继续下拉刷新
    private boolean isCanPush = true;               // 处理当前View中的各种状态时，判断是否允许上拉加载

    private boolean isForbidPull = false;           // 是否禁用下拉刷新功能
    ...
    protected int getChildDrawingOrder(int childCount, int i) {
        ...
        int bigIndex = mFooterViewIndex > mHeaderViewIndex
                ? mFooterViewIndex : mHeaderViewIndex;
        int smallIndex = mFooterViewIndex < mHeaderViewIndex
                ? mFooterViewIndex : mHeaderViewIndex;
    ...
    /**
     * show the headerView animation while the touch is relased.
     *
     * @param refreshing
     * @param notify
     */
    ...
    /**
     * 设置是否允许下拉刷新
     * this method is replaced by {@link SlidingEmptyLayout}
     *
     * @param canPull true: can pull down; false: cannot pull down
     */
    @Deprecated
    ...
    protected void onLayout(boolean changed, int left, int top, int right,
        ...
        // @modified by JeyZheng for adapting RecyclerView: replace "height" with "child.getMeasuredHeight()"
        final int childHeight = child.getMeasuredHeight() - getPaddingTop() - getPaddingBottom();
//        SpiderLogger.getLogger().d(LOG_TAG, "debug:onLayout childHeight = " + childHeight);
        // 更新目标View的位置
    ...
    public void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        // @modified by JeyZheng: height from EXACTLY to AT_MOST
        // EXACTLY: 20dp, fill_parent or match_parent
    ...
    /**
     * whether the child's view is scrolled to top
     *
     * @return true: scroll to top and show headerView, false: do not scroll to top
     */
    ...
    /**
     * whether the child's view is scrolled to bottom
     *
     * @return true: scroll to bottom and show moreView, false: do not scroll to bottom
     */
    public boolean isChildScrollToBottom() {
        ...
        if (swipeView instanceof RecyclerView) {
            RecyclerView recyclerView = (RecyclerView) swipeView;
            ...
            return false;
        } else if (swipeView instanceof AbsListView) {
            final AbsListView absListView = (AbsListView) swipeView;
            ...
            return false;
        } else if (swipeView instanceof ScrollView) {
            ScrollView scrollView = (ScrollView) swipeView;
        ...
        } else if (swipeView instanceof NestedScrollView) {
            NestedScrollView nestedScrollView = (NestedScrollView) swipeView;
...
//                SpiderLogger.getLogger().e(LOG_TAG, "onInterceptTouchEvent - ACTION_DOWN");
...
//                SpiderLogger.getLogger().e(LOG_TAG, "onInterceptTouchEvent - ACTION_MOVE");
...
//                SpiderLogger.getLogger().e(LOG_TAG, "onInterceptTouchEvent - ACTION_UP");
                mIsBeingDragged = false;
    ...
    private boolean handlerPullTouchEvent(MotionEvent ev, int action) {
        if (isForbidPull || !isCanPull) {
            return false;
        }

        switch (action) {
            case MotionEvent.ACTION_DOWN:
//                SpiderLogger.getLogger().e(LOG_TAG, "handlerPullTouchEvent - ACTION_DOWN");
                mActivePointerId = MotionEventCompat.getPointerId(ev, 0);
                mIsBeingDragged = false;
                break;

            case MotionEvent.ACTION_MOVE: {
//                SpiderLogger.getLogger().e(LOG_TAG, "handlerPullTouchEvent - ACTION_MOVE");
...
//                Log.e(LOG_TAG, "handlerPullTouchEvent - ACTION_POINTER_DOWN");
                final int index = MotionEventCompat.getActionIndex(ev);
                mActivePointerId = MotionEventCompat.getPointerId(ev, index);
                break;

            case MotionEventCompat.ACTION_POINTER_UP:
//                Log.e(LOG_TAG, "handlerPullTouchEvent - ACTION_POINTER_UP");
                onSecondaryPointerUp(ev);
                break;

            case MotionEvent.ACTION_UP:
            case MotionEvent.ACTION_CANCEL: {
//                Log.e(LOG_TAG, "handlerPullTouchEvent - ACTION_UP");
                if (mActivePointerId == INVALID_POINTER) {
                ...
                final float overscrollTop = (y - mInitialMotionY) * DRAG_RATE;
                if (overscrollTop > mTotalDragDistance) {
                    // @modified by JeyZheng for adapting RecyclerView: replace "true" with "mIsBeingDragged"
                    setRefreshing(mIsBeingDragged, true /* notify */);
                } else {
                    mRefreshing = false;
